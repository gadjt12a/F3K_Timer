...
- **Poker & Ladder target tasks (fw-v31)** — one mechanism, two sources for the target: the pilot's call (Poker) or the current rung (Ladder, advancing *only when reached*). Ladders come in two shapes and behave identically: task D climbs by a fixed step forever (`start=`/`step=`), while Big Ladder (K, 1:00/1:30/2:00/2:30/3:00) and Huge Ladder (M, 3:00/5:00/7:00) walk an explicit `rungs=` list and end when it runs out (fw-v35). ⚠ `rungs=` is deliberately not `targets=` — that already carries Poker's target *count*. The flight clock counts **down** to the target, because the timekeeper calls the remaining time to the pilot, and turns orange past it — the call is banked and the rest is wasted airtime. `TGT M:SS` shows before a throw so the pilot knows what they are flying for.
...
- **The base owns the timer while it is connected (fw-v37)** — R-hold at idle used to open the settings chain regardless of the connection, so a caller could change working time or discipline on a watch that was about to be told both by the next `TASK`. Connected, R-hold now goes **straight to Round Recall** and nothing else is reachable. ⚠ Round Recall survives deliberately: it is the timer's own record of what it flew, and the moment you most need it is when the base station is the thing that has failed.
- **Firmware is base-managed and pushed (`OTAPUSH`, fw-v37)** — the base sweeps every 20 s and updates any timer running an older build, with no button presses on the watch. Updating used to take four R-holds per timer, so in practice a fleet drifted apart — which matters, because the protocol is versioned and a mixed fleet is a mixed protocol. ⚠ **The safety gate matters more than the feature**: a push only goes out with the round machine IDLE and **no heat even loaded**, and the timer re-checks its own state on arrival, because a push can be in flight when a round starts. It never downgrades — a timer ahead of the base means the *base* is stale, and only a CD confirming the Settings downgrade button sends `force=1`, the sole path past the refusal in [I-41].
- **Time Up shows what scored, not what was longest (fw-v36)** — on Poker and Ladder rounds the "best three times" ranking was actively misleading, because only achieved calls score and a long flight that missed its call is worth nothing. Rows now read `5. 0:50.8 >0:50` — launch number, flown time, and the call — green when the call stood, dim when the launch was spent for nothing. Ordinary tasks keep the traffic light; the screen tells them apart by whether any flight carries a target.
- **Up to 20 flights, paged with L (fw-v36)** — `MAX_FLIGHTS` was 10 and a learner routinely flies 8–10. Past ten, `addFlight()` returned false and the flight vanished from the local log *and* the NVS history — silently, and only on a long round. The base station always had them. L pages the Time Up list (`9-16 of 20`) since only eight fit on the round display.
