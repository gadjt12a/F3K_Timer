...
f3k-timer/
  ...
  include/
    ...
    fw_version.h              <- auto-generated by release-firmware.ps1; defines FW_VERSION "fw-vN"
...
13. **OTA version mismatch loop** — If `firmware/ota/firmware.bin` was built with a
    ...
    Root cause: using `-SkipBuild` on the release script. Always use the script without
    `-SkipBuild` for real releases so the embedded version matches the tag.

14. **PowerShell scripts must be ASCII-only** — PowerShell 5.1 reads `.ps1` files as
...
2. Writes `include/fw_version.h` with `#define FW_VERSION "fw-vN"` **before** building
3. Builds the waveshare env (unless -SkipBuild — see warning below)
4. Copies `firmware.bin`, `bootloader.bin`, `partitions.bin` to `firmware/releases/fw-vN/`
5. Writes `release.txt` with version, date, source commit hash, and esptool flash addresses
6. Deletes the oldest release folder when more than 5 are present
7. Copies `firmware.bin` and writes `version.json` to `firmware/ota/` (base station OTA source)
8. Commits `firmware/` + `include/fw_version.h` then tags that commit `fw-vN`

> **WARNING: never use `-SkipBuild` for a release.**  The script writes `fw_version.h`
> BEFORE building so the embedded `FW_VERSION` string matches the tag. Skipping the build
> copies a binary that was compiled with the PREVIOUS version string, creating a mismatch
> that causes the OTA check to always report an update available.
